...
3. экспорт отчета (CSV/JSON); PDF намеренно отложен;
4. платежная интеграция и активация плана через провайдера;
5. UX-страница статуса подписки и управления доменами.

Уже есть в коде:

- CSV/JSON export на стороне отчёта;
- digest API для интеграций;
- удалена зависимость от PDF как обязательной части продукта.
